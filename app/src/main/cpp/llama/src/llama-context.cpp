...
void llama_context::resolve_fused_ops(const llama_memory_context_i * mctx, uint32_t n_seqs) {
    ...
    if (cparams.auto_fgdn) {
        ...
        // The device-mismatch probe above does not catch a chunked Gated Delta Net
        // path issue observed on Android for hybrid/recurrent models at n_ubatch == 1,
        // so force-disable it here (overriding the probe result) to keep decode stable.
        if (cparams.fused_gdn_ch && cparams.n_ubatch == 1 &&
                (llm_arch_is_hybrid(model.arch) || llm_arch_is_recurrent(model.arch))) {
            LLAMA_LOG_INFO("%s: fused Gated Delta Net (chunked) force-disabled for Android hybrid/recurrent model with n_ubatch = %u\n",
                    func, cparams.n_ubatch);
