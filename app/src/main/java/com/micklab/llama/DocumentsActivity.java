...
public class DocumentsActivity extends Activity {
    ...
    private String buildJapaneseManualText() {
        ...
        sb.append("- APIサーバー開始 / APIサーバー停止: APIサーバーを起動または停止します。\n");
        ...
        sb.append("- プロンプトテンプレート: システムプロンプト、Think有効/無効（chat-template-kwargs.enable_thinking）、カスタムテンプレートを設定できます。カスタム未設定時はモデルファミリーから自動選択されます。\n");
        sb.append("- Llama APIサーバー: サーバーポートを指定します。APIの起動/停止は起動時ポップアップまたはメイン画面から行います。\n");
