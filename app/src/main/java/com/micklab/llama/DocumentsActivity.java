...
public class DocumentsActivity extends Activity {
    ...
    private String buildJapaneseManualText() {
        ...
        sb.append("- MCP設定: モデル設定とは別のアプリ共通設定としてMCPコンフィグJSONを保存できます。WebUI側のMCP設定も合わせて参照され、入力欄が空欄のときはSSEを前提とした設定例がヒント表示されます。\n");
    ...
    private String buildEnglishManualText() {
        ...
        sb.append("- MCP Settings: Save MCP config JSON as an app-wide shared setting separate from model profiles. WebUI MCP settings are also referenced, and when the field is blank an SSE-oriented example is shown as a hint.\n");
    ...
    private String buildJapanesePrivacyText() {
        ...
        sb.append("- 設定情報（モデルURL、各種パラメータ、APIポート、ログレベル、MCPコンフィグJSON）\n");
        ...
        sb.append("また、アプリの生成機能やWebUIを利用する際、ユーザーが入力したテキスト（会話内容）は、端末内または同一ローカルネットワーク内で動作するローカル API に送信されます。MCPサーバーを設定して利用しない限り、このデータが外部サーバーへ送信されることはなく、保存も行いません。\n\n");
        sb.append("---\n\n");
        sb.append("2. 通信\n");
        ...
        sb.append("- MCPサーバーを設定してWebUIから利用した場合、会話内容やツール入力の一部が設定先のMCPサーバーへ送信されることがあります。送信先や取扱いは各MCPサーバーの設定・運用に依存します。\n");
    ...
    private String buildEnglishPrivacyText() {
        ...
        sb.append("- Configuration data (model URL, parameters, API port, log level, MCP config JSON)\n");
        ...
        sb.append("When using the generation features or the WebUI, the text entered by the user (conversation content) is sent to a local API running on the device or within the same local network. Unless you configure and use MCP servers, this data is not transmitted to external servers and is not stored.\n\n");
        ...
        sb.append("- If you configure and use MCP servers from the WebUI, parts of conversation content or tool inputs may be sent to those configured MCP servers. The destination and handling depend on each MCP server's own configuration and operation.\n");
