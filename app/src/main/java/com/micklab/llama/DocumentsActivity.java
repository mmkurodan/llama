...
public class DocumentsActivity extends Activity {
    ...
    private String buildJapaneseManualText() {
        ...
        sb.append("- MTP（投機デコード・実験的）: 「MTP speculative decoding」トグルで有効化します（既定OFF）。有効時は n_draft（1ステップあたりのドラフト数・既定2）とドラフト元（「このモデル自身のMTPヘッドを使用（推奨）」または別GGUF）を指定できます。設定はモデルごとのプロファイルに保存され、設定保存後のモデル読み込みで反映されます。Qwen3.5-MTP や Gemma 4 など MTPヘッドを内蔵するモデルでは「自ヘッド使用」を選ぶだけで、追加ファイル不要・メモリ二重消費なしで生成が高速化する場合があります（効果は端末・モデル・n_draft に依存。OFF時は従来と同一動作。推論中は各操作が無効化されます）。\n");
