...
public final class Translations {
    ...
    private static void registerAll() {
        e("Busy wait timeout for queued requests:", "Délai d'attente pour les requêtes en file :", "Tiempo de espera para solicitudes en cola:", "Tempo de espera para solicitações na fila:", "Wartezeit für eingereihte Anfragen:", "Timeout di attesa per le richieste in coda:", "排队请求的等待超时：", "대기 중인 요청의 대기 시간 제한:");
        e("Unlimited (wait forever)", "Illimité (attendre indéfiniment)", "Ilimitado (esperar indefinidamente)", "Ilimitado (esperar para sempre)", "Unbegrenzt (unendlich warten)", "Illimitato (attendi per sempre)", "无限制（一直等待）", "무제한(계속 대기)");
        e("Keep the device awake (instant response during sleep; uses more battery)", "Garder l'appareil actif (réponse instantanée en veille ; consomme plus de batterie)", "Mantener el dispositivo activo (respuesta instantánea en suspensión; consume más batería)", "Manter o dispositivo ativo (resposta instantânea em suspensão; consome mais bateria)", "Gerät wach halten (sofortige Reaktion im Ruhezustand; höherer Akkuverbrauch)", "Mantieni il dispositivo attivo (risposta immediata in sospensione; consuma più batteria)", "保持设备唤醒（休眠时也即时响应；更耗电）", "기기를 깨운 상태로 유지(절전 중에도 즉시 응답; 배터리 소모 증가)");
        e("Ignore battery optimization", "Ignorer l'optimisation de la batterie", "Ignorar la optimización de batería", "Ignorar a otimização de bateria", "Akku-Optimierung ignorieren", "Ignora l'ottimizzazione della batteria", "忽略电池优化", "배터리 최적화 무시");
        e("The previous model load was interrupted by an app crash.", "Le chargement précédent du modèle a été interrompu par un plantage de l'application.", "La carga anterior del modelo se interrumpió por un fallo de la aplicación.", "O carregamento anterior do modelo foi interrompido por uma falha do aplicativo.", "Das vorherige Laden des Modells wurde durch einen App-Absturz unterbrochen.", "Il caricamento precedente del modello è stato interrotto da un arresto anomalo dell'app.", "上一次模型加载因应用崩溃而中断。", "이전 모델 로드가 앱 비정상 종료로 인해 중단되었습니다.");
        e("The previous model load likely stopped because the process ran out of memory and was killed. This is not an mmap/address-space problem — lowering the GPU offload layers or the context length (n_ctx) usually helps.", "Le chargement précédent du modèle s'est probablement arrêté car le processus a manqué de mémoire et a été arrêté. Ce n'est pas un problème de mmap/espace d'adressage — réduire les couches déchargées sur le GPU ou la longueur de contexte (n_ctx) aide généralement.", "La carga anterior del modelo probablemente se detuvo porque el proceso se quedó sin memoria y fue terminado. No es un problema de mmap/espacio de direcciones: reducir las capas descargadas en la GPU o la longitud de contexto (n_ctx) suele ayudar.", "O carregamento anterior do modelo provavelmente parou porque o processo ficou sem memória e foi encerrado. Não é um problema de mmap/espaço de endereço — reduzir as camadas descarregadas na GPU ou o comprimento do contexto (n_ctx) geralmente ajuda.", "Das vorherige Laden des Modells wurde wahrscheinlich beendet, weil dem Prozess der Speicher ausging und er beendet wurde. Das ist kein mmap-/Adressraum-Problem – das Reduzieren der auf die GPU ausgelagerten Schichten oder der Kontextlänge (n_ctx) hilft in der Regel.", "Il caricamento precedente del modello si è probabilmente interrotto perché il processo ha esaurito la memoria ed è stato terminato. Non è un problema di mmap/spazio di indirizzamento — ridurre i livelli scaricati sulla GPU o la lunghezza del contesto (n_ctx) di solito aiuta.", "上一次模型加载很可能因进程内存不足被终止而停止。这不是 mmap/地址空间问题——降低 GPU 卸载层数或上下文长度 (n_ctx) 通常会有帮助。", "이전 모델 로드는 프로세스가 메모리 부족으로 종료되어 중단되었을 가능성이 높습니다. 이는 mmap/주소 공간 문제가 아니며, GPU 오프로드 레이어 수나 컨텍스트 길이(n_ctx)를 줄이면 대개 도움이 됩니다.");
        e("The previous model load was stopped by a user action or by the system.", "Le chargement précédent du modèle a été arrêté par une action de l'utilisateur ou par le système.", "La carga anterior del modelo fue detenida por una acción del usuario o por el sistema.", "O carregamento anterior do modelo foi interrompido por uma ação do usuário ou pelo sistema.", "Das vorherige Laden des Modells wurde durch eine Benutzeraktion oder durch das System gestoppt.", "Il caricamento precedente del modello è stato interrotto da un'azione dell'utente o dal sistema.", "上一次模型加载被用户操作或系统停止。", "이전 모델 로드가 사용자 작업 또는 시스템에 의해 중지되었습니다.");
        e("The previous model load did not finish (cause unknown). For large models this is most often due to running out of memory — lowering the GPU offload layers or n_ctx usually helps.", "Le chargement précédent du modèle ne s'est pas terminé (cause inconnue). Pour les grands modèles, cela est le plus souvent dû à un manque de mémoire — réduire les couches déchargées sur le GPU ou n_ctx aide généralement.", "La carga anterior del modelo no finalizó (causa desconocida). En modelos grandes suele deberse a falta de memoria — reducir las capas descargadas en la GPU o n_ctx suele ayudar.", "O carregamento anterior do modelo não terminou (causa desconhecida). Para modelos grandes, isso geralmente ocorre por falta de memória — reduzir as camadas descarregadas na GPU ou n_ctx geralmente ajuda.", "Das vorherige Laden des Modells wurde nicht abgeschlossen (Ursache unbekannt). Bei großen Modellen liegt das meist an zu wenig Speicher – das Reduzieren der auf die GPU ausgelagerten Schichten oder n_ctx hilft in der Regel.", "Il caricamento precedente del modello non è stato completato (causa sconosciuta). Per i modelli grandi ciò è spesso dovuto a memoria insufficiente — ridurre i livelli scaricati sulla GPU o n_ctx di solito aiuta.", "上一次模型加载未完成（原因不明）。对于大型模型，这通常是由于内存不足——降低 GPU 卸载层数或 n_ctx 通常会有帮助。", "이전 모델 로드가 완료되지 않았습니다(원인 불명). 대형 모델에서는 보통 메모리 부족 때문이며, GPU 오프로드 레이어 수나 n_ctx를 줄이면 대개 도움이 됩니다.");
        ...
        e("Max Output Tokens (n_predict, -1 = unlimited):", "Tokens max (n_predict, -1 = illimité) :", "Tokens máx (n_predict, -1 = ilimitado):", "Tokens máx (n_predict, -1 = ilimitado):", "Max. Ausgabetokens (n_predict, -1 = unbegrenzt):", "Token max (n_predict, -1 = illimitato):", "最大输出 token 数 (n_predict, -1 = 无限制)：", "최대 출력 토큰 수 (n_predict, -1 = 무제한):");
        e("KV Cache Quantization:", "Quantification du cache KV :", "Cuantización caché KV:", "Quantização cache KV:", "KV-Cache-Quantisierung:", "Quantizzazione cache KV:", "KV 缓存量化：", "KV 캐시 양자화:");
        e("GPU Switch Stabilization:", "Stabilisation GPU Switch :", "Estabilización GPU Switch:", "Estabilização GPU Switch:", "GPU-Switch-Stabilisierung:", "Stabilizzazione GPU Switch:", "GPU 切换稳定化：", "GPU 전환 안정화:");
